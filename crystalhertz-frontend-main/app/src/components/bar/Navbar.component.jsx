...
export const Navbar = () => {
    ...
    const pageInfoKey = JSON.parse(localStorage.getItem("last_page"))?.key;
    return (<nav className={`navbar-${typeClassName}`}>
      <ul style={{ margin: "auto auto auto 0" }}>
        {data.map((item, index) => (<Link to={`/${item.key}`} key={index} className={`navbar-${typeClassName}_list`}>
            <NavbarItemSelected className={`navbar-${typeClassName}_list-item`} onClick={() => {
...
const NavbarItemSelected = styled.span `

...
import {
  ListenerAccount,
  ListenerAccountProfile,
  ListenerDisplay,
  ListenerDescription,
  ListenerTracks,
} from "../styles/userAccount.style";

/**
 * User account sub-page
 * @returns {JSX.Element} as page-component
 */
export const ListenerAccountPage: FC = (): JSX.Element => {
  const { leftBar } = useSelector(selectAppState);
  const token = localStorage.getItem("token");

  const [isListener] = useState<boolean>(false);
    ...
    <ListenerAccount.Frame>
      ...
      <ListenerAccount.Container isOpen={leftBar.isOpen}>
        <ListenerAccountProfile.Container>
          <ListenerAccountProfile.ProfileImage
        ...
        <ListenerTracks.Frame>
          ...
          <ListenerTracks.Container>
            {_resData.map((el, index) => (
              <ListenerTracks.Item key={index}>

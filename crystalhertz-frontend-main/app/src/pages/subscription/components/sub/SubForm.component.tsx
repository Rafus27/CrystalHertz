...
type ModalType = "surprise" | "choosePayment" | "";

export const SubForm: FC = (): JSX.Element => {
  const [policyisAgree, setPolicyIsAgree] = useReducer((v) => !v, false);
  const [showModal, setShowModal] = useReducer((v) => !v, false);
  const [selectModal, setSelectModal] = useState<ModalType>("");
    ...
    <SubscriptionMenuForm.Container>
      ...
      <SubscriptionMenuForm.ContainerInner>
        ...
        <SubscriptionMenuForm.ButtonContainer>
          <SubButton
            color={"rgba(158, 4, 183, 1)"}
            message={"Купить в подарок"}
            cb={() => {
              setShowModal();
          ...
          />
          <SubButton
            color={"rgba(255, 111, 8, 0.8)"}
            message={"Выбрать способ оплаты"}
            cb={() => {
              setShowModal();
      ...
      <SubscriptionMenuForm.PolicyAgreeSubContainer>
        <SubscriptionMenuForm.Checkbox
          defaultChecked={policyisAgree}
          onChange={setPolicyIsAgree}
        />
      ...
      <SubscriptionMenuForm.TotalContainer>
        ...
        <SubscriptionMenuForm.ButtonBuy
          policyAgree={policyisAgree}
          onClick={handlerBuyingSubs}
        >
          Оплатить
        </SubscriptionMenuForm.ButtonBuy>
      </SubscriptionMenuForm.TotalContainer>

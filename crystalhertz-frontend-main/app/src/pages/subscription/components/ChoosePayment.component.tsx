...
const PaymentItem: PaymentCardData[] = [
  {
    id: 2,
    title: "Сбербанк",
...
interface DiffPaymentCardData {
  id: number;
  title: string;
  description: string;
}

const DiffPaymentItem: DiffPaymentCardData[] = [
  {
    id: 1,
    title: "В унике отдам",
 ...
 * @returns {JSX.Element}
 */
export const ChoosePayment: FC = (): JSX.Element => {
  return (
    <PaymentContainer>
      <PaymentGrid>
        {PaymentItem.map((item: PaymentCardData) => {
          return (
            <PaymentGridCard key={item.id}>
              <PaymentGridCardTop>
      ...
      <PaymentGrid>
        {DiffPaymentItem.map((item: DiffPaymentCardData) => {
          return (
            <PaymentGridCard key={item.id}>
              <p>{item.title.toUpperCase()}</p>

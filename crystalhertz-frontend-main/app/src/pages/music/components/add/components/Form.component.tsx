// lib
import { FC, MouseEvent, useReducer, useRef, useState } from "react";
 ...
 * @returns {JSX.Element}
 */
export const AddMusicForm: FC<Pick<Props, "setModalCb">> = (
  props
): JSX.Element => {
  const [agreePolicy, setAgreePolicy] = useReducer((v: boolean) => !v, false);

  const [data, setData] = useState<Omit<Props, "setModalCb">>(
    {} as Omit<Props, "setModalCb">
  );

  const inputRef = useRef<InputRef>(null);

  const onAlbumNameChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setData({ ...data, albumName: e.target.value });
    ...
    <AddMusic.Frame>
      <AddMusic.Container>
        <AddMusic.Form>
          ...
          <Input
            value={data.nameTrack}
            minLength={2}
            maxLength={16}
            onChange={(e) => setData({ ...data, nameTrack: e.target.value })}
            showCount
            bordered
          />
          ...
          <Input
            value={data.descsTrack}
            onChange={(e) => setData({ ...data, descsTrack: e.target.value })}
            maxLength={200}
            showCount
            style={{ margin: "0 auto 0 2.5px", maxWidth: "100%" }}
          />

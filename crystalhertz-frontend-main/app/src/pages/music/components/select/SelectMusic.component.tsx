// lib
import { FC, useReducer, useState, useRef, useEffect } from "react";
 ...
 */
export const SelectMusicComponent: FC<Omit<Music, "isSelected">> = (
  props
): JSX.Element => {
  ...
  const [play, setPlay] = useReducer((v: boolean) => !v, false);
  const [currentTime, setCurrentTime] = useState<string>("00:00");
  const [updatedCurrentTime, setUpdatedCurrentTime] = useState<number>(0);
  const [duration, setDuration] = useState<string>("00:00");

  useEffect(() => {
    if (play === true)
      setDuration(getDuration(audioElement.current?.duration as number));
  }, [play]);
  ...
  return (
    <SelectMusic.Container
      onClick={handleClick}
      isSelected={props.id === id ? true : false}
    >
      <audio
        ref={audioElement}
        src={props.soundSrc}
        onTimeUpdate={(e) => setCurrentTime(timeUpdate(e))}
        onLoadedMetadata={(e) => {
      ...
      <SelectMusic.Progress.Container>
        ...
        <SelectMusic.Progress.AudioInput
          type="range"
          defaultValue={0}
          maxLength={audioElement.current?.duration}
          value={audioElement.current?.currentTime}
          disabled
          onChange={(e) =>
            setUpdatedCurrentTime(getNumberFromDuration(e.target.value))
          }
        />
